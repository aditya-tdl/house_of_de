    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (modalMode === "CREATE") {
                ...
                const response = await axiosInstance.patch(`/journal/${selectedBlog?.id}`, data, {
                    headers: { "Content-Type": undefined } // Let browser set multipart/form-data with boundary
                });
                if (response.data.success) {
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-border bg-secondary/30">
                                <th className="p-4 text-[10px] uppercase tracking-widest font-medium text-muted-foreground w-16">Image</th>
                            ...
                            <AnimatePresence mode="popLayout">
                                {filteredBlogs.map((blog) => (
                                    ...
                                    >
                                        <td className="p-4">
                                            {blog.image_url ? (
                                                <div className="w-10 h-10 rounded-sm overflow-hidden flex-shrink-0 border border-border">
                                                    <img src={blog.image_url} alt={blog.title} className="w-full h-full object-cover" />
                                                </div>
                                            ) : (
                                                <div className="w-10 h-10 rounded-sm bg-secondary flex items-center justify-center flex-shrink-0 border border-border">
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                {/* Image Upload */}
                                <div>
                                    <label className="luxury-label text-[10px] block mb-2">Cover Image</label>
                                    <div
                                        onClick={() => fileInputRef.current?.click()}
                                        className="w-full h-48 border border-dashed border-border hover:border-foreground/50 bg-secondary/10 flex flex-col items-center justify-center cursor-pointer transition-colors relative overflow-hidden group"
                                    >
                                        {imagePreview ? (
                                            <>
                                                <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" />
                                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <p className="text-white text-xs uppercase tracking-widest flex items-center gap-2">
                                            ...
                                            <div className="text-center text-muted-foreground">
                                                ...
                                                <p className="text-[10px] uppercase tracking-widest">Click to upload image</p>
                                            </div>
                                        )}
                                        <input
                                            type="file"
